...
resource "azurerm_subnet" "shi" {
  ...
  resource_group_name  = azurerm_resource_group.shi.name
  virtual_network_name = azurerm_virtual_network.shi.name
...
resource "azurerm_network_interface" "shi" {
  ...
  ip_configuration {
    ...
    public_ip_address_id = azurerm_public_ip.shi.id
  }
}
...
resource "azurerm_network_interface" "shi-ir" {
  name                = "${module.naming_shi.network_interface.name}-ir"
  location            = azurerm_resource_group.shi.location
  resource_group_name = azurerm_resource_group.shi.name
  ...
  ip_configuration {
    ...
    subnet_id                     = azurerm_subnet.shi.id
    private_ip_address_allocation = "Dynamic"
    public_ip_address_id = azurerm_public_ip.shi-ir.id
  }
}

resource "azurerm_windows_virtual_machine" "shi-ir" {
  name                = "${module.naming_shi.virtual_machine.name}-ir"
  resource_group_name = azurerm_resource_group.shi.name
  location            = azurerm_resource_group.shi.location
  size                = "Standard_F2"
  admin_username      = var.admin_username
  admin_password      = var.admin_password
  network_interface_ids = [
    azurerm_network_interface.shi-ir.id,
  ]
